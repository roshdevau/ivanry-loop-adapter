...
test -n "$AUDIT_PATH"
node -e 'const fs=require("fs");const x=JSON.parse(fs.readFileSync(process.argv[1]));if(!x.beforeFingerprint||x.beforeFingerprint!==x.afterFingerprint)throw new Error("preferences changed during preview");if(!x.readOnlyRequests?.length||x.readOnlyRequests.some(r=>!["GET","HEAD","OPTIONS"].includes(r.method)))throw new Error("preview made a non-read-only financial request")' "$AUDIT_PATH"
rm -f "$RUNTIME_TARGET"
